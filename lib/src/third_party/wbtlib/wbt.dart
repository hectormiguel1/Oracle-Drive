...
import 'package:oracle_drive/src/third_party/wbtlib/wbt.g.dart' as native;
...
class WhiteBinTools {
  ...
  static List<FileEntry> getMetadata(
    ...
    using((Arena arena) {
      final fileListPathPtr = fileListPath
      ...
      NativeResult.unwrap(
        ...
        native.free_result,
        onSuccess: (res) {
          final fileEntryListPtr = res.payload.data
              .cast<native.FileEntryList>();
          ...
          for (int i = 0; i < count; i++) {
            ...
            final binChunkIndex =
                int.parse("0x${chunkInfoSections[0]}") * 0x800;
  ...
  static void repackMultipleInternal(
    ...
    using((Arena arena) {
      final fileListPtr = fileListPath
          .toNativeUtf8(allocator: arena)
          .cast<Char>();
